<template>
    <div>
        ...
        <nav-bread>
            <span slot="message">Order Confirm</span>
        </nav-bread>
                ...
                <div class="item-list-wrap confirm-item-list-wrap">
                    <div class="cart-item order-item">
                        ...
                        <ul class="cart-item-list">
                            <!-- 只有当item.checked == '1' 表示商品已选中的时候，此时遍历的数据就是订单的商品数据了-->
    ...
    export default {
        data() {
            return {
                ...
                discount: 200,                // 折扣
                tax: 400,                     // 扣税
                subTotal: 0,                 // 每件商品总金额加在一起
                orderTotal: 0,               // 订单总额  ===  subTotal + shipping - discount + tax
                cartList: []                  // cartList取出购物车中所有的商品信息
            }
        },
        mounted() {
            this.init();
        ...
        methods: {
            init() {
                axios.get("/users/cartList").then( (response) => {
                    let res = response.data;                        // 获取到购物车中的数据
                    this.cartList = res.result;

                    this.cartList.forEach((item) => {
                        if(item.checked == '1') {          // 勾选的商品
